...
def lossy_compression_algo(infl, outfl, scalefac=65535, vertchunks = 1, complev=4, numvert = 300):
    ...
    with xr.open_dataset(infl, chunks = {"lev": vertchunk_size}, cache=False) as ds:
        encoding = {}
        fvalue = -32768
        deletevars = ["nci", "ncs", "ncr", "ncg", "qg", "qc", "qr", "qs", "lenscl"] #variables to delete!
        ...
        for var in ds.data_vars:
            ...
            # Select variables to apply symlog to
            if "pert" in var.lower() or "tke" in var.lower() or "dissten" in var.lower() or var.lower()[0] == "q":
                if var.lower()[0] == "q": #convert water variables to ppmv
            ...
            #edge case for a homogeneous dataset
...
def compress_data(vertlevs, exp, timesteps):
    ...
    for typee in types:
        for tstep in timesteps: 
            ...
            #ADJUST BELOW based on your file name
...
if __name__ == '__main__':
    ...
    #this is the experiment number, which I use to name the outut file
    exp = int(sys.argv[2])
    
    # Remaining arguments are the timesteps (used to open the files)
